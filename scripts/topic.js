...
fs.createReadStream('topic2.csv')
  .pipe(csv())
  .on('data', (row) => {
    // Adjusted to include topic_name in the insert operation
    const insertPromise = client.query('INSERT INTO topic (topic_name, subject_name) VALUES ($1, $2) ON CONFLICT (topic_name) DO UPDATE SET subject_name = EXCLUDED.subject_name', [row.topic_name, row.subject_name]);
